...
size_t unwind_backtrace_to_buf(struct pt_regs *regs, struct task_struct *tsk, 
			char *buffer, size_t length)
{
	...
	while (1) {
		...
		if (urc < 0)
			break;
		len += dump_backtrace_entry_to_buf(where, frame.pc, frame.sp - 4,
					buffer + len, length - len);
		if (length == len) break; // buffer is full.
	}
	return len;
}
//Terry add for power off reason -
