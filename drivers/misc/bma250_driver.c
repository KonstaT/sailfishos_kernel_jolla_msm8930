...
struct bma250_data {
	...
	/* Detroit 3.0 CR #:XXX, WH Lee, 20121128 */
	struct bma250acc user_offset;
	/* WH Lee, 20121128 */
	/* 8930 Boston_CR #:XXX, WH Lee, 20130107 */
#ifdef CONFIG_PM_LOG
...
static int bma250_reset(struct i2c_client *client)
{
	...
	data1 = 0xB6;
	comres = bma250_smbus_write_byte(client, BMA250_RESET_REG, &data1);
...
static int bma250_set_mode(struct i2c_client *client, unsigned char Mode)
{
	...
	if (client == NULL) {
		...
		/* 8930 Boston_CR #:XXX, WH Lee, 20130107 */
#ifdef CONFIG_PM_LOG
		bma250 = i2c_get_clientdata(client);
#endif
		/* WH Lee, 20130107 */
		if (Mode < 3) {
			...
			case BMA250_MODE_NORMAL:
				...
				/* 8930 Boston_CR #:XXX, WH Lee, 20130107 */
#ifdef CONFIG_PM_LOG
				comres = pmlog_device_on(bma250->pmlog_device);
				if (comres)
					printk(KERN_ERR "%s: pmlog_device_on fail rc = %d\n", __func__,comres);
#endif
				/* WH Lee, 20130107 */
				break;
			...
			case BMA250_MODE_SUSPEND:
				...
				/* 8930 Boston_CR #:XXX, WH Lee, 20130107 */
#ifdef CONFIG_PM_LOG
				comres = pmlog_device_off(bma250->pmlog_device);
				if (comres)
					printk(KERN_ERR "%s: pmlog_device_off fail rc = %d\n", __func__,comres);
#endif
				/* WH Lee, 20130107 */
				break;
			default:
		...
		struct bma250acc *acc)
{
	...
	if (client == NULL) {
		...
		/* Detroit 3.0 CR :#XXX, WH Lee, 20121128 */
		/* Add offset */
		acc->x += bma250->user_offset.x;
		acc->y += bma250->user_offset.y;
		acc->z += bma250->user_offset.z;
		/* WH Lee, 20121128 */
	}
...
static ssize_t bma250_user_offset_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma250_data *bma250 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", bma250->user_offset.x);
}

static ssize_t bma250_user_offset_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	s16 data;
	...
	error = kstrtos16(buf, 10, &data);
	if (error)
		return error;

	bma250->user_offset.x = data;
	return count;
}

static ssize_t bma250_user_offset_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma250_data *bma250 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", bma250->user_offset.y);
}

static ssize_t bma250_user_offset_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	s16 data;
	...
	bma250->user_offset.y = data;
	return count;
}

static ssize_t bma250_user_offset_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma250_data *bma250 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", bma250->user_offset.z);
}

static ssize_t bma250_user_offset_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	s16 data;
...
static ssize_t bma250_reset_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (bma250_reset(bma250->bma250_client) < 0)
		return -EINVAL;
...
/* Detroit 3.0 CR #:XXX, WH Lee, 20121115 */
static DEVICE_ATTR(user_offset_x, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_user_offset_x_show,
		bma250_user_offset_x_store);

static DEVICE_ATTR(user_offset_y, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_user_offset_y_show,
		bma250_user_offset_y_store);

static DEVICE_ATTR(user_offset_z, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_user_offset_z_show,
		bma250_user_offset_z_store);

static DEVICE_ATTR(chip_id, S_IRUGO,
		bma250_chip_id_show, NULL);

static DEVICE_ATTR(reset, S_IWUSR | S_IWGRP,
		NULL, bma250_reset_store);
/* WH Lee, 20121115 */
...
/* Detroit 3.0 CR #:XXX, WH Lee, 20121115 */
	&dev_attr_user_offset_x.attr,
	&dev_attr_user_offset_y.attr,
	&dev_attr_user_offset_z.attr,
		...
		const struct i2c_device_id *id)
{
	...
	struct bma250_platform_data *pdata = dev->platform_data;
	...
	/* 8930 Boston_CR #:XXX, WH Lee, 20130107 */
#ifdef CONFIG_PM_LOG
	data->pmlog_device = pmlog_register_device(&client->dev);
#endif
	/* WH Lee, 20130107 */
...
error_sysfs:
	/* 8930 Boston_CR #:XXX, WH Lee, 20130107 */
#ifdef CONFIG_PM_LOG
	if (data->pmlog_device)
		pmlog_unregister_device(data->pmlog_device);
#endif
	/* WH Lee, 20130107 */
