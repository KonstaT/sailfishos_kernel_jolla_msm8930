...
/* Add pm log */
#ifdef CONFIG_PM_LOG
extern struct pmlog_device *wcnss_pmlog_device;
#endif
/* WH Lee, 20130114 */
...
int wcnss_req_power_on_lock(char *driver_name)
{
	...
	/* Add pm log */
#ifdef CONFIG_PM_LOG
	if (pmlog_device_on(wcnss_pmlog_device))
		pr_err("%s: pmlog_device_on fail\n", __func__);
#endif
	/* WH Lee, 20130114 */
...
int wcnss_free_power_on_lock(char *driver_name)
{
	...
	/* Add pm log */
#ifdef CONFIG_PM_LOG
	if (pmlog_device_off(wcnss_pmlog_device))
		pr_err("%s: pmlog_device_off fail\n", __func__);
#endif
	/* WH Lee, 20130114 */
