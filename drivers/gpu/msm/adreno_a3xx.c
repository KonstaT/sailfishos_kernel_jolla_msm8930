...
static void set_gmem_copy_quad(struct adreno_device *adreno_dev)
{
        ...
        memcpy(adreno_dev->scratch_quad_vertices_restore.hostptr, gmem_restore_quad,
                QUAD_RESTORE_LEN << 2);
}

static void a3xx_create_scratch_cmds(struct adreno_device *adreno_dev)
{
       unsigned int *cmds;

       cmds = adreno_dev->scratch_cmd.hostptr;
       ...
       *cmds++ = (HLSQ_TP_MIPMAP_BASE << CP_LOADSTATE_STATETYPE_SHIFT)
               | (0 << CP_LOADSTATE_EXTSRCADDR_SHIFT);
       *cmds++ = adreno_dev->scratch_pad.gpuaddr;
       *cmds++ = 0x00000000;
       *cmds++ = 0x00000000;
       *cmds++ = 0x00000000;
       *cmds++ = 0x00000000;
       ...
       *cmds++ = _SET(VFD_FETCHINSTRUCTIONS_FETCHSIZE, 7) |
       ...
       /* VFD_FETCH_INSTR_1_0 */
       *cmds++ = _SET(VFD_BASEADDR_BASEADDR,
               adreno_dev->scratch_quad_vertices_restore.gpuaddr);
       /* VFD_FETCH_INSTR_0_1 */
       *cmds++ = _SET(VFD_FETCHINSTRUCTIONS_FETCHSIZE, 11) |
               _SET(VFD_FETCHINSTRUCTIONS_BUFSTRIDE, 12) |
               _SET(VFD_FETCHINSTRUCTIONS_INDEXDECODE, 1) |
       ...
       /* VFD_FETCH_INSTR_1_1 */
       *cmds++ = _SET(VFD_BASEADDR_BASEADDR,
               adreno_dev->scratch_quad_vertices_restore.gpuaddr + 16);
       ...
       *cmds++ = CP_REG(A3XX_RB_RENDER_CONTROL);
       /* RB_RENDER_CONTROL */
       *cmds++ = _SET(RB_RENDERCONTROL_BIN_WIDTH, 1 >> 5) |
               _SET(RB_RENDERCONTROL_ALPHA_TEST_FUNC, 7);
       ...
       *cmds++ = 0x00000002; /* Num indices */
       ...
       adreno_dev->scratch_ib[0] = CP_HDR_INDIRECT_BUFFER_PFD;
       adreno_dev->scratch_ib[1] = adreno_dev->scratch_cmd.gpuaddr;
       adreno_dev->scratch_ib[2] = cmds - (unsigned int *)adreno_dev->scratch_cmd.hostptr;
}
			 ...
			 struct adreno_ringbuffer *rb)
{
	...
	if (soc_class_is_msm8930())
		a3xx_create_scratch_cmds(adreno_dev);
}
